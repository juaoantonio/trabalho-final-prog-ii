...
class OrderTest {
	...
	void shouldRoundPercentageDiscountWithTwoDecimalsHalfUp() {
		...
		Coupon c =
				Coupon.builder()
						...
						.build(); // 10% = 3.30
		o.applyCoupon(c);
		assertEquals(0, o.getDiscountTotal().getValue().compareTo(new Money(3.30).getValue()));
		assertEquals(0, o.getTotalAmount().getValue().compareTo(new Money(29.70).getValue()));
	}
	...
	void shouldApplyFixedCoupon() {
		...
		assertEquals(0, order.getDiscountTotal().getValue().compareTo(new Money(10).getValue()));
		assertEquals(0, order.getTotalAmount().getValue().compareTo(new Money(20).getValue()));
	}
	...
	void shouldApplyPercentageCoupon() {
		...
		Coupon coupon =
				Coupon.builder()
						.code("DESCONTO10")
						.type(CouponType.PERCENT)
						.value(new Money(10)) // 10%
						.isActive(true)
						.build();
		order.applyCoupon(coupon);
		assertEquals("DESCONTO10", order.getCouponCode().get());
		assertEquals(0, order.getDiscountTotal().getValue().compareTo(new Money(10).getValue()));
		assertEquals(0, order.getTotalAmount().getValue().compareTo(new Money(90).getValue()));
	}
	...
	void shouldReplacePreviousCouponWhenReapplied() {
		...
		Coupon c1 = Coupon.builder().type(CouponType.FIXED).value(new Money(10)).isActive(true).build();
		Coupon c2 =
				Coupon.builder().type(CouponType.PERCENT).value(new Money(20)).isActive(true).build();
		o.applyCoupon(c1);
		o.applyCoupon(c2);
		assertEquals(0, o.getDiscountTotal().getValue().compareTo(new Money(20).getValue()));
		assertEquals(0, o.getTotalAmount().getValue().compareTo(new Money(80).getValue()));
	}

	@Test
	@DisplayName("Deve recalcular desconto e total após adicionar itens com cupom percentual aplicado")
	void shouldRecalculateDiscountAndTotalWhenAddingItemsWithPercentCoupon() {
		Order o = newPendingOrder();
		o.addItem(newItem(100, false)); // subtotal 100
		Coupon c =
				Coupon.builder().type(CouponType.PERCENT).value(new Money(10)).isActive(true).build();
		o.applyCoupon(c); // desconto 10
		o.addItem(newItem(50, false)); // subtotal 150
		assertEquals(0, o.getDiscountTotal().getValue().compareTo(new Money(15).getValue()));
		assertEquals(0, o.getTotalAmount().getValue().compareTo(new Money(135).getValue()));
	}
	...
	void shouldIgnoreFixedCouponWithZeroValue() {
		...
		Coupon c = Coupon.builder().type(CouponType.FIXED).value(new Money(0)).isActive(true).build();
		o.applyCoupon(c);
		assertEquals(0, o.getTotalAmount().getValue().compareTo(new Money(40).getValue()));
	}
	...
	void shouldNotApplyCouponWhenOrderIsPaid() {
		Order order = Order.builder().withId(UUID.randomUUID()).withStatus(OrderStatus.PAID).build();
		Coupon coupon =
				Coupon.builder()
						.code("DESCONTO10")
						.type(CouponType.FIXED)
